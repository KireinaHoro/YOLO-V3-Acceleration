...
__global__ void upsample_gpu(int N, const float *x, int w, int h, int c,
                             int batch, int zoomFactor, float *out,
  ...
  if (mode == "bilinear") {
    ...
    out[out_index] = (1 - u) * (1 - v) * x[in_index00] +
                     (1 - u) * v * x[in_index01] + u * (1 - v) * x[in_index10] +
                     u * v * x[in_index11];
...
void interp_gpu(const float *x, int w, int h, int c, int batch, int zoomFactor,
  ...
  upsample_gpu<<<cuda_gridsize(outSize), BLOCK, 0, stream>>>(
      outSize, x, w, h, c, batch, zoomFactor, out);
}
