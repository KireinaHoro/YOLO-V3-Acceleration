...
void interp_gpu(const float *x, int w, int h, int c, int batch, int zoomFactor,
                float *out, cudaStream_t stream);

template <int zoomFactor> class Interp : public IPlugin {
public:
  Interp() {}
  ...
  Dims getOutputDimensions(int index, const Dims *inputs,
    ...
    mOutputDims = DimsCHW(inputs[index].d[0], inputs[index].d[1] * zoomFactor,
                          inputs[index].d[2] * zoomFactor);
    if (0) {
      std::cout << "IPlugin input dim = [" << inputs[index].d[0] << ", "
                << inputs[index].d[1] << ", " << inputs[index].d[2] << "]"
                << std::endl;
      std::cout << "IPlugin output dim = [" << mOutputDims.d[0] << ", "
  ...
  void configure(const Dims *inputs, int nbInputs, const Dims *outputs,
                 int nbOutputs, int maxBatchSize) override {
    assert(1 == nbInputs && 1 == nbOutputs);
    mInputDims = inputs[0];
    mInputSize =
        inputs[0].d[0] * inputs[0].d[1] * inputs[0].d[2] * sizeof(float);
    // mOutputSize = outputs[0].d[0] * outputs[0].d[1] * outputs[0].d[2] *
    // sizeof(float);
  }
  size_t getWorkspaceSize(int) const override { return 0; }

  // @ when serializing the engine
  size_t getSerializationSize() override { return sizeof(mInputDims); }
  ...
  int enqueue(int batchSize, const void *const *inputs, void **outputs, void *,
    ...
    interp_gpu(
        (const float *)inputs[0], mInputDims.d[2], mInputDims.d[1],
        mInputDims.d[0], batchSize, zoomFactor, (float *)outputs[0],
        stream); // TODO: didnt serialize mInputDims, can we use it? in that
                 // case, i serialized mInputDims, instead of mInputSize.
    return 0;
  }
...
public:
  ...
  IPlugin *createPlugin(const char *layerName, const void *serialData,
    ...
    if (!strcmp(layerName, "Interp85")) {
      assert(mPluginInterp85.get() == nullptr);
      mPluginInterp85 = std::unique_ptr<Interp<ZOOM>>(
          new Interp<ZOOM>(serialData, serialLength));
      return mPluginInterp85.get();
    } else if (!strcmp(layerName, "Interp97")) {
      assert(mPluginInterp97.get() == nullptr);
      mPluginInterp97 = std::unique_ptr<Interp<ZOOM>>(
          new Interp<ZOOM>(serialData, serialLength));
      return mPluginInterp97.get();
    } else {
      assert(0);
