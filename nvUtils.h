...
inline bool CHECK_(CUresult e, int iLine, const char *szFile) {
  if (e != CUDA_SUCCESS) {
    LOG_ERROR(logger, "CUDA error " << e << " at line " << iLine << " in file "
                                    << szFile);
    return false;
  }
  return true;
...
inline bool CHECK_(cudaError_t e, int iLine, const char *szFile) {
  if (e != cudaSuccess) {
    LOG_ERROR(logger, "CUDA runtime error " << e << " at line " << iLine
                                            << " in file " << szFile);
    return false;
  }
  return true;
...
inline bool CHECK_(NVENCSTATUS e, int iLine, const char *szFile) {
  if (e != NV_ENC_SUCCESS) {
    LOG_ERROR(logger, "NVENC error " << e << " at line " << iLine << " in file "
                                     << szFile);
    return false;
  }
  return true;
...
inline bool CHECK_(HRESULT e, int iLine, const char *szFile) {
  if (e != S_OK) {
    LOG_ERROR(logger, "HRESULT error " << e << " at line " << iLine
                                       << " in file " << szFile);
    return false;
  }
  return true;
...
inline bool CHECK_(GLenum e, int iLine, const char *szFile) {
  if (e != 0) {
    LOG_ERROR(logger, "GLenum error " << e << " at line " << iLine
                                      << " in file " << szFile);
    return false;
  }
  return true;
...
public:
    ...
    double Stop() {
        struct timeval t1;
                ...
                return (t1.tv_sec - t0.tv_sec) + (t1.tv_usec -
t0.tv_usec)/1000000;
        }
