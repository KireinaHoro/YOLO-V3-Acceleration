...
// nms involved two sorting, sorting for all bboxes (per Image) and sorting for
// all bboxes per class
 ...
 *
 *  TODO:
 *  1. Since the array is same length, consider move the temp storage size
 * outside for reuse.
 *  2. Add return value
 */
void sortScoresPerImage_gpu(const int nBatch, const int nItemsPerImage,
                            void *unsorted_scores, void *unsorted_bbox_indices,
                            void *sorted_scores, void *sorted_bbox_indices,
                            void *workspace,
  ...
  void *cubWorkspace =
      nextWorkspacePtr((int8_t *)d_offsets, (nBatch + 1) * sizeof(int));
  ...
  size_t temp_storage_bytes =
      cubSortPairsWorkspaceSize<float, int>(arrayLen, nBatch);

  // enough workspzce
  assert(temp_storage_bytes <=
         (maxSizeofWorkspaceInByte - (nBatch + 1) * sizeof(int)));

  // sort with cub
  cub::DeviceSegmentedRadixSort::SortPairsDescending(
      cubWorkspace, temp_storage_bytes, (const float *)(unsorted_scores),
      (float *)(sorted_scores), (const int *)(unsorted_bbox_indices),
      (int *)(sorted_bbox_indices), arrayLen, nBatch, (const int *)d_offsets,
      (const int *)d_offsets + 1, 0, sizeof(float) * 8, stream);
}

/**
 * get bboxes data from prediction and archor (biases)
 */
__device__ box get_region_box(float x_in, float y_in, float w_in, float h_in,
                              int i_in, int j_in, float w, float h,
...
__launch_bounds__(nthds_per_cta) __global__ void splitOutputData_kernel(
    const int nBatch, // batch
    const int nClasses,
    const int nBboxesPerLoc, // #box
    const int coords,        // x,y,w,h
    const int l0_w, const int l0_h, const int nCells,
    const bool background,      // use background conf or not
    const bool only_objectness, // no class conf
    const float thres, const float *predictions, const float *biases,
    float *probes, box *bboxes) {
  ...
  if (cur_idx < totalBboxes) {
    ...
    int scaleIdx = batchIdx * batchMemLen + bboxIdx * bboxMemLen +
    ...
    bboxes[outBboxIdx] = get_region_box(
        predictions[baseIdx], predictions[baseIdx + nCells],
        predictions[baseIdx + 2 * nCells], predictions[baseIdx + 3 * nCells],
        col, // column (i), nchw
        row, // row (j), nchw
        (1 << locLayer) * l0_w, (1 << locLayer) * l0_h,
...
/**
 */
void splitOutputData_gpu(const int nBatch, // batch
                         const int nClasses,
                         const int nBboxesPerLoc, // #box
                         const int coords,        // x,y,w,h
                         const int l0_w, const int l0_h, const int nCells,
                         const bool background, // use background conf or not
                         const bool only_objectness, // no class conf
                         const float thres, const float *predictions,
                         const float *biases, float *probes, box *bboxes,
                         cudaStream_t stream) {
  const int blockSize = 512;
  const int gridSize =
      (nBatch * nBboxesPerLoc * nCells + blockSize - 1) / blockSize;
  splitOutputData_kernel<blockSize><<<gridSize, blockSize, 0, stream>>>(
      nBatch, nClasses, nBboxesPerLoc, coords, l0_w, l0_h, nCells, background,
      only_objectness, thres, predictions, biases, probes, bboxes);
}
 ...
 *  1. Resize with original ratio, and keep long side the same size with
 * net_input (w or h).
 *  2. Add black space in the short side equally.
 *  e.g. origial image = (832, 320), network input size = (416, 416).
 *       First resize the image to (416, 160), then adding 416*128 black pixels
 * above and below the resized image.
 */
template <unsigned nthds_per_cta>
__launch_bounds__(nthds_per_cta) __global__
    void correct_region_boxes_kernel(const int nBatch, // batch
                                     const int nClasses,
                                     const int nBboxesPerLoc, // #box
                                     const int nCells, const int image_w,
                                     const int image_h, const int net_input_w,
                                     const int net_input_h, box *bboxes) {
  size_t i = blockIdx.x * nthds_per_cta + threadIdx.x;

  if (i < nBatch * nBboxesPerLoc * nCells) {
    bboxes[i] = correct_region_box(bboxes[i], image_w, image_h, net_input_w,
                                   net_input_h);
  }
}

void correct_region_boxes_gpu(const int nBatch, // batch
                              const int nClasses,
                              const int nBboxesPerLoc, // #box
                              const int nCells, const int image_w,
                              const int image_h, const int net_input_w,
  ...
  correct_region_boxes_kernel<blockSize><<<gridSize, blockSize, 0, stream>>>(
      nBatch, nClasses, nBboxesPerLoc, nCells, image_w, image_h, net_input_w,
      net_input_h, bboxes);
}
 ...
 *
 * With split data before, the data is splited into boxes and probes, and probes
 *can be sorted per class
 *
 * * ! prob is in order batch -> classes -> box -> loc, so the interveal is
 *nCells*nBboxesPerLoc
 * * output: only sorted index is needed and tmp_probes is needed
 *
 * TODO:
 * a. how to solve batched issue?
 *   * solution 1 (easiest): create temp GPU memory outside, use memory copy to
 *put all prob together (as yolov2 darknet code)
 *   * solution 2 (easiest): create temp GPU memory in function (yolov2 darknet
 *code)
 *   * solution 2: call cub sort batchSize times
 *   * solution 3 (best): use a larger arrayLen, skip first 5 channels (x, y, w,
 *h, conf) for cub sort. b. add return value
 **/
void sortScoresPerClass_gpu(const int nBatch, const int nClasses,
                            const int nBboxesPerImage, const void *probes,
                            void *sorted_boxIdx, void *workspace,
                            const size_t maxSizeofWorkspaceInByte,
                            cudaStream_t stream) {
  // using solution 1
  ...
  // initiate boxIndex
  void *unsorted_boxIdx =
      nextWorkspacePtr((int8_t *)sorted_probes, arrayLen * sizeof(float));
  setUniformOffsets(arrayLen - 1, 1, (int *)unsorted_boxIdx, stream);

  // initiate offset
  void *d_offsets =
      nextWorkspacePtr((int8_t *)unsorted_boxIdx, arrayLen * sizeof(int));
  ...
  size_t temp_storage_bytes =
      cubSortPairsWorkspaceSize<float, int>(arrayLen, nSegments);
  ...
  cub::DeviceSegmentedRadixSort::SortPairsDescending(
      cubWorkspace, temp_storage_bytes, (const float *)(probes),
      (float *)(sorted_probes), (const int *)(unsorted_boxIdx),
      (int *)(sorted_boxIdx), arrayLen, nSegments, (const int *)d_offsets,
      (const int *)d_offsets + 1, 0, sizeof(float) * 8, stream);
}
 ...
 */
template <int TSIZE>
__global__ void allClassNMS_kernel(const int nBatch, // batch
                                   const int nClasses, const int nBboxesPerLoc,
                                   const int nCells, const float nms_threshold,
                                   const box *bboxes, const float *probes,
                                   float *afterNMS_probes, const int *indexes,
  ...
  const int offset_indexes =
      batchIdx * nClasses * sizeOfClass + classIdx * sizeOfClass;
...
#pragma unroll
  for (int t = 0; t < TSIZE; ++t) {
    ...
    if (itemIdx_indexes < maxIdx_indexes) {
      ...
      if (bboxIdx_tSize[t] != -1 && abs(probes[probIdx_probes]) > 1e-30) {
        const int bboxIdx_bbox =
            batchIdx * sizeOfClass + bboxIdx_tSize[t] % sizeOfClass;
  ...
  int refBboxIdx_bbox =
      batchIdx * sizeOfClass + indexes[refItemIdx_indexes] % sizeOfClass;
  ...
  while (refItemIdx_indexes < maxIdx_indexes) {
    ...
    for (int t = 0; t < TSIZE; ++t) {
      ...
      if ((keptBboxInfoFlag_loc[curIdx_loc]) &&
          (itemIdx_indexes > refItemIdx_indexes)) {
...
#pragma unroll
  for (int t = 0; t < TSIZE; ++t) {
    ...
    if (readItemIdx_probes < maxIdx_indexes) {
      afterNMS_probes[writeItemIdx_indexes] =
          keptBboxInfoFlag_loc[curIdx_loc] ? probes[readItemIdx_probes] : 0.0f;
      afterNMS_indexes[writeItemIdx_indexes] =
          keptBboxInfoFlag_loc[curIdx_loc] ? bboxIdx_tSize[t] : -1;
 ...
 */
void allClassNMS_gpu(const int nBatch, // batch
                     const int nClasses, const int nBboxesPerLoc,
                     const int nCells, const float nms_threshold, void *bboxes,
                     void *probes, void *afterNMS_probes, void *indexes,
                     void *afterNMS_indexes, cudaStream_t stream) {
#define P(tsize) allClassNMS_kernel<(tsize)>

  void (*kernel[32])(const int, const int, const int, const int, const float,
                     const box *, const float *, float *, const int *,
                     int *) = {P(1),  P(2),  P(3),  P(4),  P(5),  P(6),  P(7),
  ...
  kernel[t_size -
         1]<<<gridSize, blockSize, blockSize * t_size * sizeof(bool), stream>>>(
      nBatch, nClasses, nBboxesPerLoc, nCells, nms_threshold,
      (const box *)bboxes, (const float *)probes, (float *)afterNMS_probes,
      (const int *)indexes, (int *)afterNMS_indexes);
}

size_t getWorkspaceSizeInByte(const int nBatch, const int nClasses,
                              const int nBboxesPerLoc, const int nCells) {
  // 1. temporary storage of sortScoresPerClass_GPU
  size_t mem_for_sortScoresPerClass = 0;
  // 1.1 sorted probes
  mem_for_sortScoresPerClass =
      nBatch * nClasses * nBboxesPerLoc * nCells * sizeof(float);
  // 1.2 unsorted bbox
  mem_for_sortScoresPerClass +=
      nBatch * nClasses * nBboxesPerLoc * nCells * sizeof(int);
  // 1.3 offset
  mem_for_sortScoresPerClass += (nBatch * nClasses + 1) * sizeof(int);
  // 1.4 cub workspace
  mem_for_sortScoresPerClass += cubSortPairsWorkspaceSize<float, int>(
      nBatch * nClasses * nBboxesPerLoc * nCells, nBatch * nClasses);
  ...
  // 2.2 cub workspace
  mem_for_sortScoresPerImage += cubSortPairsWorkspaceSize<float, int>(
      nBatch * nClasses * nBboxesPerLoc * nCells, nBatch);

  size_t maxWorkSpaceInByte =
      (mem_for_sortScoresPerClass > mem_for_sortScoresPerImage)
          ? mem_for_sortScoresPerClass
          : mem_for_sortScoresPerImage;
