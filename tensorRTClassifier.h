  ...
  const void *readCalibrationCache(size_t &length) override {
    ...
    if (input.good())
      std::copy(std::istream_iterator<char>(input),
                ...
                std::back_inserter(vCalibrationCache_));
...
public:
  ...
  void caffeToTensorRTModel(const char *deployFile, const char *modelFile,
                            ICaffeParser *parser);
  void initInfer();

  // override
  void setInputData(float *pBGR, const int nWidth, const int nHeight,
...
private:
  ...
  nvcaffeparser1::IPluginFactory *pPluginFactory_{
      nullptr}; // factory for plugin layers
