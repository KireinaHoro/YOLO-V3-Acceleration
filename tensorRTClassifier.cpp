...
TensorRTClassifier::TensorRTClassifier(
    ...
    const std::string &inputs, const std::vector<std::string> &outputs,
    const int maxBatchSize, const int devID,
    nvcaffeparser1::IPluginFactory *pPluginFactory, std::string table)
...
void TensorRTClassifier::setInputData(float *pBGR, const int nWidth,
                                      const int nHeight, const int nBatchSize) {
  ...
  ck(cudaMemcpy(apBuffers_[inputIndex_], pBGR, inputSize_,
...
void TensorRTClassifier::forward(INFER_OUTPUT_PARAMS *pInferOutputParams) {
  ...
  // pInferOutputParams->dpInferResults_ = reinterpret_cast<float*
  // >(apBuffers_[outputIndex_]);
  for (int j = 0; j < nOutputs_; ++j) {
    pInferOutputParams->vpInferResults_[j] =
        reinterpret_cast<float *>(apBuffers_[vOutputIndexs_[j]]);
    pInferOutputParams->vnLens_[j] =
        vOutputDims_[j].c() * vOutputDims_[j].h() * vOutputDims_[j].w();
    pInferOutputParams->vOutputDims_[j] = vOutputDims_[j];
  }
}

int TensorRTClassifier::getInferWidth() const { return inputDim_.w(); }

int TensorRTClassifier::getInferHeight() const { return inputDim_.h(); }
...
void TensorRTClassifier::initInfer() {
  ...
  inputDim_ =
      static_cast<DimsCHW &&>(pEngine_->getBindingDimensions(inputIndex_));
  inputSize_ = maxBatchSize_ * inputDim_.c() * inputDim_.h() * inputDim_.w() *
               sizeof(float);
  ...
  for (int i = 0; i < nOutputs_; ++i) {
    ...
    vOutputDims_[i] = static_cast<DimsCHW &&>(
        pEngine_->getBindingDimensions(vOutputIndexs_[i]));
    vOutputSizes_[i] = maxBatchSize_ * vOutputDims_[i].c() *
                       vOutputDims_[i].h() * vOutputDims_[i].w() *
                       sizeof(float);
  }
  ...
  if (pMeanBlob_) {
    const float *meanData =
        reinterpret_cast<const float *>(pMeanBlob_->getData());
...
void TensorRTClassifier::caffeToTensorRTModel(const char *deployFile,
  ...
  const IBlobNameToTensor *blobNameToTensor =
      parser->parse(deployFile, modelFile, *network, DataType::kFLOAT);
