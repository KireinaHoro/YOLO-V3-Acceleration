 ...
 *  for b in batch:
 *      for g in groups:
 *          softmax(input + b*batchOffset + g*groupOffset, n, temp, stride,
 * output + b*batchOffset + g*groupOffset)
 *
...
__launch_bounds__(nthdsPerCTA) __global__
    void softmaxKernel(const float *input, const int n, const int batch,
                       const int batchOffset, const int groups,
                       const int groupOffset, const int stride,
                       const float temp, float *output) {
  ...
  if (id < batch * groups) {
    ...
    for (int i = 0; i < n; ++i) {
      float e = exp(input[i * stride + offset] / temp -
                    largest / temp); // bound score in (-inf,0], and denominator
                                     // fractor in (0,1].
      sum += e;
...
template <unsigned nthdsPerCTA>
__launch_bounds__(nthdsPerCTA) __global__
    void activateKernel(float *data, const int range) {
 ...
 *
 * If we integrated into tensorRT, we can use input and output are different
 * memory. If it is standalone GPU code (in main.cpp), we can use input and
...
void regionLayer_gpu(const int batch, const int C, const int nCells,
  ...
  // for YOLOv3, the output of final layer is C*nCells, in which, C includes all
  // the conf, coord, and claesses.

#ifdef REGION_IN_TRT
  // TRT, input and output are diff buffer
  ck(cudaMemcpy((void *)output, (void *)input,
                batch * C * nCells * sizeof(float), cudaMemcpyDeviceToDevice));
#endif
  ...
  for (int b = 0; b < batch; ++b) {
    for (int n = 0; n < num; ++n) {
      // activate on (x,y)
      int index =
          b * C * nCells // per batch
          +
          n * nCells * (coords + classes + 1); // coords, classes and confidence
      activateKernel<blockSize>
          <<<gridSize_Act1, blockSize, 0, stream>>>(output + index, 2 * nCells);

      // activate on probes on conf
      index = b * C * nCells + n * nCells * (coords + classes + 1) +
              4 * nCells; // skip coords
      activateKernel<blockSize>
          <<<gridSize_Act2, blockSize, 0, stream>>>(output + index, nCells);

      // softmax for all classes
      index = b * C * nCells + n * nCells * (coords + classes + 1) +
              5 * nCells; // skip conf
      softmaxKernel<blockSize><<<gridSize_Softmax, blockSize, 0, stream>>>(
...
template <unsigned nthdsPerCTA>
__launch_bounds__(nthdsPerCTA) __global__
    void reorgOutputKernel(const int nBatch, const int nClasses,
                           const int nBboxesPerLoc, const int coords,
                           const int l0_w, const int l0_h, const int nCells,
                           float *dpData_unordered[], float *dpData) {
  ...
  if (i < range) // voc<266175 coco<904995 wrt. 416*416 input
  {
    ...
    dpData[i] =
        dpData_unordered[locLayer]
                        [b * nBboxesPerLoc * (nClasses + coords + 1) *
                             (1 << locLayer) * (1 << locLayer) * l0_w * l0_h +
                         bboxIdx * (nClasses + coords + 1) * (1 << locLayer) *
                             (1 << locLayer) * l0_w * l0_h +
                         channelIdx * (1 << locLayer) * (1 << locLayer) * l0_w *
                             l0_h +
...
void reorgOutput_gpu(const int nBatch, const int nClasses,
  ...
  reorgOutputKernel<blockSize><<<gridSize, blockSize, 0, stream>>>(
      nBatch, nClasses, nBboxesPerLoc, coords, l0_w, l0_h, nCells,
      dpData_unordered, dpData);
}
